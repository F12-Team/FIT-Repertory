...
def direction(direction_id):
    ...
    cursor.execute('SELECT id, name FROM Projects WHERE id_of_direction = %s ORDER BY likes DESC limit 10;', (direction_id,))
...
def all_projects(direction_id):
    ...
    cursor.execute('SELECT id, name FROM Projects WHERE id_of_direction = %s ORDER BY likes DESC;', (direction_id,))
    projects = cursor.fetchall()
    cursor.close()
